...
static bool stepModel(void) {
    ...
    for (i = self_handler->inputSize; i < self_handler->inputArraySize; i++) {
        self_handler->handler->input->data.f[i] = self_handler->handler->output->data.f[i];
    }
...
static bool setupModel(const unsigned char* model_data, size_t arena_size, int inputArraySize, int outputArraySize, int inputSize, int outputSize) {
